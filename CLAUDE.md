...
- **RAG Query**: `chatRequest` расширен полями `ragDocIds`, `ragStrategy`, `ragTopK`. В `handleChat` при наличии `ragDocIds`: embed query через Ollama → `SearchChunks()` с auto-стратегией (ищет по всем 4 стратегиям, возвращает лучшие top-K) → XML `<documents>` inject в user message → SSE `rag_context` event. `POST /api/docs/{id}/search` — standalone endpoint для поиска. Frontend: `activeRagDocIds` в chat store, секция "Documents" в `SendSettingsPopover` (чекбоксы ready-документов, emerald подсветка gear icon), `RAGSearchResult` в `MessageBubble` (сворачиваемый блок chunks с score). Ограничение: nomic-embed-text слабо работает кросс-лингвально.
